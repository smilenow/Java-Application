...
public class SearchStudent {
	...
	public static void main(String[] args) {
		...
		for (Record x:database)
			if (x.name.equals(what_to_input)) answer.add(x);
		...
		if (answer.size() > 0){
			...
			System.out.println("+---------------+---------------+");
			for (Record x:answer){
				System.out.println("|\t"+x.course+"\t|\t"+x.mark+"\t|");
				total += x.mark;
			}
			System.out.println("+---------------+---------------+");
			avg = total*1.0/answer.size();
			System.out.println("Total course(s): "+answer.size());
			System.out.printf("Average marks: %.2f\n",avg);
		}
		else System.out.println("Cannot find student called :"+what_to_input);
	}
